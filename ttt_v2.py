...
if __name__ == '__main__':
    file_no = input('input_no:')
    count = int(input('input_count(1~100):'))
    aid  =  input('input_article:')
    readRandomStr(file_no)
    i = 0
    sampleArray = random.sample(abclist,count)
    for ssid in sampleArray:
        if i < count:
